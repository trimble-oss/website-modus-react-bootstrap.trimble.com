...
<CodeBlock title="Icon Only Buttons " code={IconsOnlyButtons}>
...
<CodeBlock title="Small Buttons" code={SmallButtons}></CodeBlock>

<CodeBlock title="Large Buttons" code={LargeButtons}></CodeBlock>

<CodeBlock title="Full-width (Block) Buttons" code={FullWidthButtons}>
  Create block level buttons—those that span the full width of a parent—by
  adding <code>block</code>.
</CodeBlock>

<h2>Special Use Cases</h2>
<CodeBlock title="Dark Backgrounds" code={DarkBackgrounds}>
  For specific use cases where a light colored button is needed against a dark
  background use a tertiary solid, outline, or text button.
</CodeBlock>

<p>Use case examples for light buttons.</p>
<CodeBlock code={LightButtons}></CodeBlock>

<CodeBlock title="Danger Button" code={DangerButtons}>
  A danger button should be used for permanent, destructive actions like delete.
</CodeBlock>

<CodeBlock title="Yellow button" code={YellowButtons}>
  A yellow button for the Trimble brand color is provided for special use cases.
</CodeBlock>
